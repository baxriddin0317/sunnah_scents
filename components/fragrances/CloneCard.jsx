...
const CloneCard = ({product}) => {
  return (
    <div className='max-w-52 sm:max-w-72 mx-auto w-full bg-white border border-brand-gray min-h-full h-full'>
      <div className='px-4 py-8'> 
        <div className='inline-block text-nowrap bg-brand-main text-white text-sm lg:text-lg font-medium capitalize px-2 py-1 mb-4'>20% Closeness</div>
        <div className='relative h-[198px]'>
          <Image className='absolute object-contain' src={`/products/product-${product.id}.png`} fill alt={product.name} />
        </div>
